...
//     var myDiv = document.getElementById('final_settings');
//     var scrollAmount = Math.max(0, (215 - window.pageYOffset));
//     myDiv.style.top = scrollAmount + 'px';
// };
...
$('#new_file').change(function () {
    ...
    if (file) {
        ...
        $('#new_image').attr('src', "data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==");
    }
});
...
$(document).ready(function () {
    ...
    $("#advanced_option").on('click', () => {
        isAdvancedOption = !isAdvancedOption;
        if (isAdvancedOption == true) {
            $("#advanced_setting").removeClass("hidden");
        } else {
            $("#advanced_setting").addClass("hidden");
        }
    });
...
$(document).on('click', '.step', function () {
    ...
    if (Math.floor(index / 2) < CUR_STATE.current_step) {
        CUR_STATE.current_step = Math.floor(index / 2);
    ...
    if (stream_type == 'backend') {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                messages: [{
                    role: 'user',
                    content: `Generate ${keywords_count} keywords(simple words or 2 words, not phrase, not person name) about '${keywords_topic}'. Must resut as array json data. Result format is [keyword1, keyword2, ..., keywordn].`
                }],
                model: openai_model
            }),
            success: function (data) {
...
function generateTitles() {
    ...
    if (stream_type == 'backend') {
        ...
        let content = title_topic.trim() == '' ? `Generate ${titles_count} titles(Maximum title length is ${title_length}. Must not be 'title1', 'title2', 'title3', 'title4', 'title5') about Keywords: '" . ${wizardData.keywords} . "'. Resut must be array json data. This is result format: [title1, title2, ..., titlen]. Maximum title length is ${title_length}` : `Generate ${titles_count} titles(Maximum title length is ${title_length}., Must not be 'title1', 'title2', 'title3', 'title4', 'title5') about Topic: '" . ${title_topic} . "'. Resut must be array json data. This is result format: [title1, title2, ..., titlen]. Maximum title length is ${title_length}`;
        $.ajax({
            type: "post",
            url: atob(guest_id),
            headers: {
...
function generateOutlines() {
    ...
    if(stream_type == 'backend') {
        ...
        let content = topic_outline.trim() == '' ? `The keywords of article are ${wizardData.keywords}.  Generate different outlines( Each outline must has only ${subtitle_count} subtitles(Without number for order, subtitles are not keywords)) ${outlines_count} times. The depth is 1.  Must not write any description. Result must be json data, Every subtitle is sentence or phrase string. This is result format: [[subtitle1(string), subtitle2(string), subtitle3(string), ... , subtitle-${outlines_count}(string)], [subtitle1(string), subtitle2(string), subtitle3(string), ... , subtitle-${outlines_count}(string)], ... ,[subtitle1(string), subtitle2(string), subtitle3(string), ..., subtitle-${outlines_count} (string)].` :
         `The subject of article is ${topic_outline}. Generate different outlines( Each outline must has only ${subtitle_count} subtitles(Without number for order, subtitles are not keywords)) ${outlines_count} times. The depth is 1" . " Must not write any description. Result must be json data, Every subtitle is sentence or phrase string. This is result format: [[subtitle1(string), subtitle2(string), subtitle3(string), ... , subtitle-${subtitle_count}(string)], [subtitle1(string), subtitle2(string), subtitle3(string), ... , subtitle-${subtitle_count}(string)], ... ,[subtitle1(string), subtitle2(string), subtitle3(string), ..., subtitle-${subtitle_count} (string)]].`;
        $.ajax({
            type: "post",
            url: atob(guest_id),
            headers: {
...
async function generateArticle() {
    ...
    let nIntervId = setInterval(function () {
        if (chunk.length == 0 && !streaming) {
            ...
            $("#saved_documents").removeClass("hidden");
    ...
    const stopGenerating = () => {
        if(stream_type == 'backend') eventSource.close();
        clearInterval(nIntervId)
        isGenerating = false;
        streaming = false;
        $("#result_title").addClass("hidden");
        ...
        let asyncGenerate = async () => {
            const response = await fetch(atob(guest_id), {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `Write Article(Maximum  ${length} words). in ${language}. Generate article (Must not contain title, Must Mark outline with <h3> tag) about $title with following outline${JSON.parse(wizardData.outline).join(",")}. Must mark outline with <h3> tag. `,
                    }],
...
function updateData() {
    ...
    if (wizardData.current_step == 4) {
        ...
        $("#final_settings").removeClass("hidden");
        $("#final_settings_empty").removeClass("hidden");
        $("#wizard_area").addClass("hidden");
        $("#result_area").removeClass("hidden");

    } else {
        $("#settings").removeClass("hidden");
        $("#final_settings").addClass("hidden");
        $("#final_settings_empty").addClass("hidden");
        $("#wizard_area").removeClass("hidden");
        $("#result_area").addClass("hidden");
    }

    $(".step").each(function (index, element) {
        if (Math.floor(index / 2) >= wizardData.current_step) {
            $(element).removeClass('hover:bg-[#eeeeee]');
        } else {
            $(element).addClass('hover:bg-[#eeeeee]');
        }
        if (Math.floor(index / 2) == wizardData.current_step) {
            if (index % 2 == 0) {
    ...
    let origin_classname = "absolute h-2 progress_bar rounded-full "
    if (wizardData.current_step == 0) {
        origin_classname += 'w-1/4';
    } else if (wizardData.current_step == 1) {
        origin_classname += 'w-1/2';
    } else if (wizardData.current_step == 2) {
        origin_classname += 'w-3/4';
    } else if (wizardData.current_step == 3) {
        origin_classname += 'w-full';
    }
    ...
    $(".new_data").each(function (index, element) {
        if (index == wizardData.current_step) {
    ...
    $(".select_title_right").each(function (index, element) {
        if (index == wizardData.current_step) {
            $(element).removeClass('hidden');
        } else {
            $(element).addClass('hidden');
    ...
    $('#progress_bar').attr('class', origin_classname);
    ...
    if (wizardData.current_step == 0) {
        ...
        const selectedKeywordTemplate = document.querySelector('#selected_keyword').content.cloneNode(true);
        ...
        extra_keywords.filter(item => item.trim() != '').forEach((extra_keyword) => {
            ...
            if (keywordArr.includes(extra_keyword)) {
                ...
                new_selected_keyword.querySelector('label').textContent = (extra_keyword);
                ...
                new_unselected_keyword.querySelector('label').textContent = (extra_keyword);
        ...
        extra_titles.filter(item => item.trim() != '').forEach((extra_title) => {
            if (extra_title == title) {
                ...
                new_selected_title.querySelector('h3').textContent = (extra_title);
                ...
                new_unselected_title.querySelector('h3').textContent = (extra_title);
            ...
            if (JSON.stringify(extra_outline) == outline) {
                ...
                extra_outline.forEach((item) => {
                    if (item.trim() != "") {
                        ...
                        new_outline_sub.querySelector("label").textContent = item;
